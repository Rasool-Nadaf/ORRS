...
if(isset($_GET['ids'])){
    ...
    $qry = $conn->query("SELECT r.*, s.code as sched_code, s.train_id, s.route_from, s.route_to FROM `reservation_list` r 
        INNER JOIN `schedule_list` s ON r.schedule_id = s.id 
        WHERE r.id IN ({$_GET['ids']}) ");
    
    if($qry->num_rows > 0){
        $res = $qry->fetch_all(MYSQLI_ASSOC);
    } else {
        echo '<script> alert("Unknown Reservation ID(s).");location.replace("./?page=schedules") </script>';
    }
} else {
            ...
            <div id="outprint" class="list-group">
                ...
                <div class="list-group-item tickets py-5">
                    ...
                    <div class="row">
                        ...
                        <div class="row">
    <div class="col-auto pr-2"><b>From:</b></div>
    <div class="col-auto flex-grow-1 border-bottom border-dark">
        <b><?= isset($row['route_from']) ? $row['route_from'] : "N/A" ?></b>
    </div>
    <div class="col-auto pr-2"><b>To:</b></div>
    <div class="col-auto flex-grow-1 border-bottom border-dark">
        <b><?= isset($row['route_to']) ? $row['route_to'] : "N/A" ?></b>
    </div>
</div>
                    </div>
